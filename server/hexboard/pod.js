...
var environment = {
  listOptions: _.defaults({
    url: 'https://' + config.get('OPENSHIFT_SERVER') + '/api/namespaces/' + config.get('NAMESPACE') + '/pods'
  , auth: {bearer:  config.get('OAUTH_TOKEN')}
  , pool: listWatchAgent
  }, optionsBase)
, watchOptions: _.defaults({
    url: 'https://' + config.get('OPENSHIFT_SERVER') + '/api/watch/namespaces/' + config.get('NAMESPACE') + '/pods'
  , auth: {bearer:  config.get('OAUTH_TOKEN')}
  , pool: listWatchAgent
  }, optionsBase)
